...
public class BankRegister extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		m_parameterWhere.append(" And fa.C_AcctSchema_ID = " + p_C_AcctSchema_ID);
		setDateAcct();
	...
	protected String doIt()
	{
		createBalanceLine();
		createDetailLines();
		int AD_PrintFormat_ID = DB.getSQLValue(get_TrxName(), "Select AD_PrintFormat_ID from AD_PrintFormat Where name = 'Registro Banco alge'");
	...
	private void createBalanceLine()
	{
		StringBuffer sb = new StringBuffer ("INSERT INTO T_BankRegister "
			...
			+ "AmtAcctDr, AmtAcctCr, Balance,C_AcctSchema_ID) ");
		...
		sb.append("SELECT ").append(getAD_PInstance_ID()).append(", ").append(Env.getAD_Client_ID(getCtx())).append(", ").append(Env.getAD_Org_ID(getCtx())).append(", ")
			...
			+ "COALESCE(SUM(fa.AmtAcctDr),0), COALESCE(SUM(fa.AmtAcctCr),0), COALESCE(SUM(fa.AmtAcctDr-fa.AmtAcctCr),0), max(fa.C_AcctSchema_ID) "
			+ "From Fact_Acct fa "
			...
			if (p_C_AcctSchema_ID !=0) {
				sb.append(" And fa.C_AcctSchema_ID = " + p_C_AcctSchema_ID);
			}
	...
	private void createDetailLines()
	{
		...
		sb.append("SELECT DISTINCT ").append(getAD_PInstance_ID()).append(", fa.AD_Client_ID, fa.AD_Org_ID, ")
			...
			+ "fa.AmtAcctDr, fa.AmtAcctCr, fa.AmtAcctDr-fa.AmtAcctCr, fa.C_AcctSchema_ID "
			+ "From Fact_Acct fa "
			+ "Inner Join C_Payment p On p.C_Payment_ID = fa.Record_ID AND p.docstatus IN ('CO', 'CL') "
			+ "Inner Join C_BankAccount ba On ba.C_BankAccount_ID = p.C_BankAccount_ID "
			+ "Inner Join C_Bank b On ba.C_Bank_ID = b.C_Bank_ID "
		...
		sb.append(" order by fa.AD_Client_ID, fa.AD_Org_ID, b.name, bp.name, p.documentNo, TRUNC(fa.DateAcct),fa.C_AcctSchema_ID  ");
